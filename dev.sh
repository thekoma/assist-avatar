...
# `includes:`) when the YAML config_hash changes. Editing only the .h files (the
# engine headers under components/avatar/) leaves the config_hash unchanged, so a
# plain `esphome run` reports "up to date" and launches the OLD binary.
#
