...
template<typename D>
void render(D &it, int phase, uint32_t now_ms) {
  it.fill(black());
  ...
  switch (phase) {
    case LISTENING: {
      ...
      // white-cyan core. Particles are staggered (the ring is always evenly fed,
      // no global pulse), ease inward (smoothstep) and fade to zero at both ends
      // so the loop has no visible "teleport" reset.
      glow_ring(it, cx, cy, 40.0f, 1.5f, 4.0f, cyan(120));
      const int n = 28;
    ...
    case THINKING: {
      ...
      for (int i = 0; i < n; ++i) {
        for (int t = 0; t < trail; ++t) {
          ...
          glow_disc(it, ax, ay, 2.4f, 4.5f, cyan((uint8_t) (255 * decay)));
          glow_disc(it, bx, by, 2.2f, 4.0f, cyan((uint8_t) (215 * decay)));
        }
      }
      break;
    }
    case REPLYING: {
      ...
      for (int x = 0; x < w; ++x) {                  // x < w: column w is off-screen
        ...
        for (int dyi = -5; dyi <= 5; ++dyi) {  // vertical glow around the curve
          ...
          if (h <= 0.0f) continue;
          it.draw_pixel_at(x, yy, scale(cyan(235), h * h * env));
        }
      }
      break;
    }
    case ERROR: {
      // Soft amber pulse — gentle, never harsh red.
    ...
    case MUTED: {
      ...
      glow_ring(it, cx, cy, 36.0f, 1.5f, 3.0f, cyan(40));
    ...
    case BOOTING: {
      ...
      glow_ring(it, cx, cy, 36.0f, 1.5f, 3.0f, cyan(45));
      float p = wrap01(now_ms, 1600.0f);
      glow_arc(it, cx, cy, 36.0f, 2.0f, 4.0f, cyan(230), -(float) M_PI / 2.0f,
               2.0f * (float) M_PI * p);
      break;
    }
    case NO_WIFI: {
      ...
      for (int k = 0; k < waves; ++k) {
        ...
        glow_ring(it, cx, cy, r, 1.5f, 3.0f, cyan((uint8_t) (200 * (1.0f - p))));
      }
      break;
    }
    case NO_HA: {
      ...
      // scanning around it.
      glow_ring(it, cx, cy, 38.0f, 1.5f, 3.0f, cyan(45));
      float rot = (float) now_ms * 0.0035f;
      glow_arc(it, cx, cy, 38.0f, 2.0f, 4.0f, cyan(235), rot, 0.9f);
      break;
    }
    case IDLE:
    default: {
      ...
      glow_ring(it, cx, cy, r, 2.0f, 5.0f, cyan(b));
      break;
    }
  }
}
